...
> **Kept verbatim as the instructions the AI tooling actually ran under**,
> including the parts the finished build contradicts. Editing it afterwards to
> match the result would make it a description rather than a record.
>
> Three things below were overridden during the build, each deliberately and
> each documented:
>
> - **"Do not add … Docker Compose."** The build machine had the wrong Ruby and
>   no Node, so containers were the cheaper option rather than the more
>   elaborate one. See `docs/decisions.md` 8.
> - **TanStack Table** is pinned in the stack table below and was dropped:
>   sorting and pagination are server-side, so it rendered nothing and did no
>   work. See `web/README.md` § 4.
> - **"Never cut … the deployed URL."** It was cut anyway. See the
>   out-of-scope section of `docs/requirements.md`.
