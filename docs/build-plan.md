...
> **This is the plan as written before the build, kept unedited.** What
> actually happened is recorded at the bottom under *Outcome* — the plan is
> more useful as a record of intent than as a tidied-up description of the
> result.
...
33 commits. The phase order held; the deviations were these.

**Step 33 (deploy) was not done.** No hosting account was created. The
single-process setup from step 32 is built and verified — Rails serves the
bundle and the API from one origin — so deployment is configuration rather
than a code change. Recorded in `requirements.md` as an explicit cut.

**Step 35 (demo video) is outstanding.** Not something the build could do.

**Steps 12 and 13 were merged.** `CLAUDE.md` requires tests to land with the
code they cover, which is how every other numbered step is already
structured; splitting auth into code-then-tests contradicted it.

**CSV export was cut** — first on the cut list and the only item taken from
it. Employee create and update shipped as API endpoints without a UI.

**Three things in the supporting docs turned out to be wrong** and were
corrected rather than implemented as written: the FX conversion formula in
`architecture.md` lost a cent to floating point, the analytics cache
watermark missed employee-only changes, and the `(employee_id,
effective_from DESC)` index was redundant. Each is documented where it was
wrong.

**TanStack Table was dropped.** Sorting and pagination are server-side, so it
rendered nothing and did no work.

**Two defects were found by screenshotting the running app** rather than by
tests: a blank employee detail page, and Rails refusing requests that carried
a container hostname. Neither was visible to typechecking or to 521 passing
tests.

### Acceptance checklist, as measured

- [x] `rspec` green, under 30 seconds, run three times with the same result —
      491 examples, 10–12s each run
- [x] `rubocop`, `brakeman`, `bundler-audit` clean
- [x] `rails db:reset` from scratch produces a working 10k dataset
- [ ] Deployed URL — cut, see above
- [x] `config/master.key` and any `.env` gitignored and absent from `git log -p`
- [x] Commit history reads as a sequence of decisions, with `docs:` first
- [x] README covers setup, tests, the demo login and the architecture
