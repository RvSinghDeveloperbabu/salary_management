...
## 8. Docker Compose, despite CLAUDE.md forbidding it

**Context.** `CLAUDE.md` lists Docker Compose among the things not to add,
on the grounds that it costs time the schedule does not have. The build uses
it anyway, and a reviewer will see that contradiction, so it is recorded here
rather than left to look like an oversight.

**Decision.** Both services run in containers, orchestrated by
`compose.yaml`. Ruby 3.4 and Node 22 are pinned by image.

**Why.** The constraint was written before the build machine was known. That
machine has Ruby 4.0.5 against a pinned 3.4, and no Node at all. The choice
was not "containers versus no containers" — it was "containers versus
installing and version-managing two language runtimes on someone's laptop".
Containers were the cheaper option, not the more elaborate one, and they make
`docker compose up` a complete setup for a reviewer with neither runtime.

**Cost.** One more thing between the reader and the code, and a first run that
pulls images. Both are paid once.

**Would change if.** The target machine already had the pinned runtimes, or
the deliverable were a library rather than a running system.

---

## 9. Sorting the directory by salary normalises currency first

**Context.** Salaries are stored in the employee's local currency. The
directory is sortable by salary.

**Decision.** The sort joins the snapshot exchange rate and orders on
`amount_cents * rate_ppm`, not on `amount_cents`.

**Why.** Found by reading the directory output rather than by design:
₹2,400,000 is about $28,800, but its integer is 24× larger than $100,000's.
Sorting a multi-country directory by "highest paid" was ranking the
lowest-paid people first — a wrong answer that looks like a right one, which
is the worst kind.

**Cost.** One more join on the sort path, and an employee whose currency has
no seeded rate sorts last rather than being excluded. The join is LEFT for
exactly that reason: a gap in reference data should be visible, not a filter.

**Would change if.** Never. Comparing unlike currencies as bare integers is
